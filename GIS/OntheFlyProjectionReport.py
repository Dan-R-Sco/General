# compares layers SR compared to the dataframes spatial reference
...
try:
                ...
                for df in arcpy.mapping.ListDataFrames(mxd):
                    ...
                    if len(layerList) > 0:  # Write the data frame name once
                        ...
                        for lyr in layerList:  # Write each layer name
                            try:
                                outFile.write("\n")
                                outFile.write("\t\t Layer: " + lyr.encode('utf-8') + "spatial reference: " + arcpy.Describe(lyr).spatialReference + "\n")
                            except:
                                outFile.write("\t\t Layer: " + lyr.encode('utf-8') + "spatial reference undeterminable")
    ...
    os.startfile(outputreplace)
    # Delete variable references
    del folderPath, output, outputreplace, outFile, fullpath
